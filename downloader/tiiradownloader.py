...
def main(
    ...
    if year_recorded:
        dl.options = YearOptions(ALUE=str(bl_area), year=year_recorded, RAJOITUS=RAJOITUS).get()
        dl.download_period()
        print(dl.options)
        csv_filename = f"{csv_filename.split('.')[0]}_tallenusvuosi_{year_recorded}.csv"
        print(csv_filename)
    elif days:
    ...
    for chance in range(2):
        try:
            print(datetime.now(), f"\tLadataan csv-tiedosto {days} päivän ajalta")
            csv = dl.download()
